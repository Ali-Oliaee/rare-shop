...
export default function AllProductsTable() {
  ...
  const handleChange = (event) => {
    const requestedCategory = event.target.value
  ...
  const findCategoryName = (id) => {
    const requestedCategoryObject = Categories.find((el) => el.id === id)
    return requestedCategoryObject?.name
  }
  const findSubCategoryName = (id) => {
    const requestedSubCategoryObject = subCategories.find((el) => el.id === id)
